...
function Message({id, text}) {
	...
	return (
		<li className="flex items-center justify-between gap-2 border px-2">
			<span className="font-bold italic">{text}</span>
			<div className="flex items-center">
				<img draggable={false} onClick={handleEdit} src={EditIcon} className="size-8"/>
				<img draggable={false} onClick={handleDelete} src={DeleteIcon} className="size-8"/>    
			</div>
